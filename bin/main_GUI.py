...
from crawl_youtube_comments import YCrawler
...
class YCrawlerUI(QDialog):
    def __init__(self):
        ...
        self.valid_youtube_url_pattern = r'(https?://)?(www\.)?youtu\.be/([a-zA-Z0-9_-]{11})|(https?://)?(www\.)?youtube\.com/watch\?v=([a-zA-Z0-9_-]{11})(&\S*)?'
        self.valid_youtube_rendering_url = None
    ...
    def init_gui(self):
        ...
        self.setWindowTitle('유투브 댓글 다운로드')
    
        self.main_v_layout = QVBoxLayout()

        # 검색어와 관련된 위젯 정렬
        self.search_h_layout = QHBoxLayout()
        self.search_label = QLabel("영상 링크 URL: ")
        self.search_label.setFixedWidth(80)
        self.search_line_edit = QLineEdit("예: https://www.youtube.com/watch?v=-6S01AeFp4k")
        ...
        self.max_num_label = QLabel("최대 댓글 수: ")
        self.max_num_line_edit = QLineEdit("예: 100개")
    ...
    # 입력된 URL 문자열이 유튜브 URL인지 확인
    def is_valid_youtube_url(self, search_key):
        if '예:' in self.search_line_edit.text().strip().split():
            search_key = ''.join(e for e in search_key.split()[1:])
        self.valid_youtube_rendering_url = search_key
        return re.match(self.valid_youtube_url_pattern, search_key) is not None

    def accept(self):  
        if self.search_line_edit.text().strip() == '':
                QMessageBox.warning(self, "경고 메세지", "검색할 유투브 URL를 입력해주세요!")
                return
        if self.max_num_line_edit.text().strip() == '':
            QMessageBox.warning(self, "경고 메세지", "다운로드 받을 댓글 개수를 입력해주세요!")
            return
        ...
        if re.findall(r'\d+', self.max_num_line_edit.text().strip()) == []:
            QMessageBox.warning(self, "경고 메세지", "다운받을 댓글 개수에 정수값을 입력해주세요!")
            return  
        if not self.is_valid_youtube_url(self.search_line_edit.text().strip()):
            QMessageBox.warning(self, "경고 메세지", "정확한 유투브 동영상 URL 주소를 입력해주세요!")
            return  
